...
class TestMatchCreation:
    ...
    def test_broadcaster_can_create_match(self, broadcaster_client, mock_blockchain):
        res = broadcaster_client.post(MATCHES_URL, {
    ...
    def test_brand_cannot_create_match(self, brand_client):
        res = brand_client.post(MATCHES_URL, {
            "team_a": "Team A", "team_b": "Team B",
            "venue": "Stadium", "match_date": "2026-05-01", "match_time": "19:00:00",
        })
        assert res.status_code == status.HTTP_403_FORBIDDEN

    def test_unauthenticated_cannot_create_match(self, api_client):
        res = api_client.post(MATCHES_URL, {
            "team_a": "Team A", "team_b": "Team B",
            "venue": "Stadium", "match_date": "2026-05-01", "match_time": "19:00:00",
        })
    ...
    def test_match_blockchain_failure_rolls_back(self, broadcaster_client, mock_blockchain):
        fail_tx = MagicMock()
        ...
        res = broadcaster_client.post(MATCHES_URL, {
            "team_a": "Team A", "team_b": "Team B",
            "venue": "Stadium", "match_date": "2026-05-02", "match_time": "19:00:00",
        })
        assert res.status_code == status.HTTP_400_BAD_REQUEST
...
class TestEventConfig:
    ...
    def test_broadcaster_can_configure_event(self, broadcaster_client, match, mock_blockchain):
        res = broadcaster_client.post(event_config_url(match.id), {
    ...
    def test_all_8_event_types_can_be_configured(self, broadcaster_client, match, mock_blockchain):
        for et in range(8):
            res = broadcaster_client.post(event_config_url(match.id), {
                "event_type": et,
                "reserve_price": "10000",
            ...
            assert res.status_code == status.HTTP_201_CREATED, f"Failed for event_type={et}"

    def test_duplicate_event_type_rejected(self, broadcaster_client, match, event_config):
        res = broadcaster_client.post(event_config_url(match.id), {
    ...
    def test_event_config_not_allowed_after_open(self, broadcaster_client, open_match):
        res = broadcaster_client.post(event_config_url(open_match.id), {
    ...
    def test_brand_cannot_configure_events(self, brand_client, match):
        res = brand_client.post(event_config_url(match.id), {
            "event_type": 0, "reserve_price": "10000",
            "reservation_fee_pct": "3", "slot_count": 2, "max_triggers": 5,
        })
        assert res.status_code == status.HTTP_403_FORBIDDEN
...
class TestOpenBidding:
    ...
    def test_broadcaster_can_open_bidding(self, broadcaster_client, match, event_config, mock_blockchain):
...
class TestExclusionGroups:
    ...
    def test_broadcaster_can_create_group(self, broadcaster_client, brand, brand2, mock_blockchain):
        res = broadcaster_client.post(EXCLUSION_GROUPS_URL, {
            "name": "Cola Wars",
    ...
    def test_brand_cannot_create_exclusion_group(self, brand_client):
        res = brand_client.post(EXCLUSION_GROUPS_URL, {
            "name": "Group", "brand_ids": [], "separation_distance": 1,
        })
        assert res.status_code == status.HTTP_403_FORBIDDEN
    ...
    def test_can_add_brand_to_group(self, broadcaster_client, broadcaster, brand):
        ...
        res = broadcaster_client.post(exclusion_group_members_url(group.id), {
            "brand_id": brand.id
        })
        assert res.status_code == status.HTTP_200_OK
    ...
    def test_locked_group_cannot_be_modified(self, broadcaster_client, broadcaster, brand):
        group = ExclusionGroup.objects.create(
            name="Locked Group", broadcaster=broadcaster,
            separation_distance=1, is_locked=True
        )
        res = broadcaster_client.post(exclusion_group_members_url(group.id), {
            "brand_id": brand.id
        })
    ...
    def test_group_locks_when_match_opens(self, broadcaster_client, broadcaster, brand, match, event_config, mock_blockchain):
        group = ExclusionGroup.objects.create(
...
class TestSimulator:
    ...
    def test_admin_can_complete_match(self, admin_client, active_match, mock_blockchain):
...
class TestSimulatorTriggerEvent:
    ...
    def test_trigger_event_creates_auction_result(
        self, admin_client, active_match, brand, approved_creative, deposit, mock_blockchain
    ):
        # Place a bid first
        bid = Bid.objects.create(
            match=active_match, brand=brand, event_type=0,
            amount=Decimal("100000"), creative=approved_creative,
            tx_hash="0x" + "b" * 64,
        )
        res = admin_client.post(simulator_trigger_url(active_match.id), {"event_type": 0})
        assert res.status_code == status.HTTP_200_OK
    ...
    def test_trigger_unconfigured_event_type_rejected(self, admin_client, active_match):
        res = admin_client.post(simulator_trigger_url(active_match.id), {"event_type": 7})
        assert res.status_code == status.HTTP_400_BAD_REQUEST

    def test_trigger_exceeds_max_triggers_rejected(self, admin_client, active_match, mock_blockchain):
...
class TestSimulatorStatus:
    ...
    def test_admin_can_view_status(self, admin_client, active_match):
        ...
        assert "trigger_counts" in data or "event_configs" in data or res.json()["success"] is True
...
class TestAuctionResults:
    ...
    def test_auction_results_grouped_by_event_type(self, api_client, completed_match, brand):
        AuctionResult.objects.create(
            match=completed_match, event_type=0, trigger_number=1,
            slot_position=0, winner=brand, amount=Decimal("100000"),
            creative_ref="https://cdn.example.com/ad.mp4", tx_hash="0x" + "a" * 64,
        )
        AuctionResult.objects.create(
            match=completed_match, event_type=0, trigger_number=1,
            slot_position=1, winner=brand, amount=Decimal("80000"),
            creative_ref="https://cdn.example.com/ad2.mp4", tx_hash="0x" + "b" * 64,
        )
        res = api_client.get(auction_results_url(completed_match.id))
        assert res.status_code == status.HTTP_200_OK
